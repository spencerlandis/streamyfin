...
5. Create an expo dev build by running `bun run ios` or `bun run android`. This will open a simulator on your computer and run the app.

For the TV version suffix the bun commands with `:tv`.

`bun run prebuild:tv`  
`bun run ios:tv or npm run android:tv`
